...
typedef struct simple_metadata_t {
  ...
  //|size| indicates the size of the free slot or the object
  //|next| allocated->unused / free -> next free slot
} simple_metadata_t;
...
void simple_add_to_free_list(simple_metadata_t* metadata) {
  ...
  //今までsimple_heapのfirst free slotをmetadataのnext(next free slot)にする
  //新たなsimple_heapのfirst free slotをmetadataにする
...
void simple_remove_from_free_list(simple_metadata_t* metadata,
  ...
  //metadata:取り除くもの
  //prev:取り除くものの1つ前のfree                              
  if (prev) {//もし1つ前が存在したら、prev->nextはmetadataからmetadata->nextになる
    prev->next = metadata->next;
  } else {//1つ前が存在しない=metadataがfree_headの場合は、metadata->nextがfree_headになる
...
void simple_initialize() {
  simple_heap.free_head = &simple_heap.dummy;//最初をdummyにする
...
void* simple_malloc(size_t size) {
  ...
  while (metadata && metadata->size < size) {
    ...
    //metadataをfree_headから順々に見ていく。
    //metadata.sizeが必要なsizeより大きくなった->use
    //使うのはこの時のmetadata
    //もし適するものがなかったらmetadata=NULLでif (!metadata)に引っかかる
  }
...
void my_initialize() {//the same as the simple_initialize
  simple_heap.free_head = &simple_heap.dummy;//最初をdummyにする
  simple_heap.dummy.size = 0;
  simple_heap.dummy.next = NULL;
}
...
void* my_malloc(size_t size) {
  ...
  simple_metadata_t* min_metadata = simple_heap.free_head;
  simple_metadata_t* min_prev = NULL;

  ////**** best fit.ver****
  
  int found = 0;//if found the just fit, found = 1
  int check=0;//if found 
  while(metadata){
    if(metadata->size==size){
      found = 1;
      break;
    }else if(min_size>metadata->size&&metadata->size>=size){
      min_prev = prev;
  ...
  if(found){//remove from free list without careing remaining size
    void* ptr = metadata + 1;
    simple_remove_from_free_list(metadata, prev);
    return ptr;
  }
  if(check){//the most fit place
    prev = min_prev;
    metadata = min_metadata;

  }
  ////**** worst fit.ver ****
  ////I dont use this version
  // size_t max_size = size;
  // simple_metadata_t* max_metadata = simple_heap.free_head;
  ...
  //     assert(max_metadata==metadata);
  //     check++;
  //   }
  //   prev = metadata;
  //   metadata = metadata->next;
  // }
  // if(check!=0){//if there was free slot available, check != 0
  //   prev = max_prev;
...
/////////////////////////////////////////////////////////////
//フリースロットの連結の部分は上手く実装できたか分かりませんが、メモリの使用率が下がってしまいました。
//スコア用に記録を取るときにはコメントオフにしました。
  // I want to connect successive free slot
  // ... | metadata | free slot | metadata | free slot | ...
  //     ^          ^           ^
  //     connect   ptr         metadata
  //         ↓
  // ... | metadata | 　　　 free slot 　　　　　　　　　　| ...
  //     ^          ^          
  //     connect   ptr   
  // connectをsimple_remove_from_free_listで取り除き、connect+metadataにしたものをsimple_add_to_free_listで足す
  ...
  simple_metadata_t* connect = simple_heap.free_head;
  simple_metadata_t* connect_prev = NULL;

  while(connect){//free listの中身を一つずつ見る
    void* ptr = connect + 1;
    if(connect->size+ptr == metadata){//check if connect and metadata are successive
      simple_remove_from_free_list(connect,connect_prev);
      //アドレスはconnectでサイズはconnect+metadataにしたい   
      connect->size = connect->size+metadata->size+sizeof(simple_metadata_t);
      connect->next=NULL;

      simple_add_to_free_list(connect);

      return;
    }
    connect_prev=connect;
